/* learn-data-engineering-with-phoebe - the Daybreak source system + warehouse setups
   Daybreak is the same DTC coffee-subscription brand from learn-sql-with-phoebe.
   Here its 6 OLTP tables play the role of the SOURCE SYSTEM: raw, text-typed
   dates and all. The course builds Daybreak's first warehouse on top.

   Every live playground re-seeds a fresh copy before running your SQL, so
   nothing you type can break the next example.

   Source schema (6 tables, dates kept as TEXT on purpose - that is the raw
   reality staging has to fix):
   ...
   window.DW_SETUPS are named warehouse states a playground can start from via
   data-setup="staging" | "star" | "scd" | "big" (they stack: star includes
   staging; scd includes star). */
...
/* Named warehouse states. Later bundles include earlier ones so a page can
   jump straight to the layer it teaches. */
window.DW_SETUPS = {};

window.DW_SETUPS.staging = `
...
SELECT product_id, name, category, price, roast FROM products;
...
SELECT sub_id, customer_id, product_id,
       ...
       CAST(cancel_date AS DATE) AS cancel_date, monthly_qty
FROM subscriptions;
`;

window.DW_SETUPS.star = window.DW_SETUPS.staging + `
...
window.DW_SETUPS.scd = window.DW_SETUPS.star + `
CREATE TABLE dim_customer_scd AS
SELECT row_number() OVER (ORDER BY customer_id) AS customer_key,
       customer_id, name, city, country, plan,
...
/* A deliberately big table for the performance sessions: ~300k synthetic
   order events generated on the fly (nothing shipped over the network). */
window.DW_SETUPS.big = `
CREATE TABLE big_orders AS
SELECT (r % 15) + 1                                        AS customer_id,
       (r % 8) + 1                                         AS product_id,
       DATE '2025-11-01' + INTERVAL ((r % 240)) DAY        AS order_date,
       CASE WHEN r % 37 = 0 THEN 'refunded' ELSE 'completed' END AS status,
       CASE WHEN r % 2 = 0 THEN 'web' ELSE 'app' END       AS channel,
       (r % 3) + 1                                         AS quantity,
       ROUND(9 + (r % 26), 2)                              AS unit_price
FROM range(300000) t(r);
`;
